...
const Home = () => {
  const celebrationItems = [
    { title: "Birthday Gifts", image: "/pngegg (6).png" },
  ...
  const gifts = [
    { id: 1, label: "UNDER", price: "₹999", bg: "bg-pink-500", textColor: "text-white" },
    { id: 2, label: "UNDER", price: "₹1499", bg: "bg-beige-100", textColor: "text-pink-500" },
    { id: 3, label: "UNDER", price: "₹2499", bg: "bg-pink-500", textColor: "text-white" },
    { id: 4, label: "PREMIUM", price: "GIFTS", bg: "bg-beige-100", textColor: "text-pink-500" },
  ];
  ...
  return (
    <>
            ...
            {celebrationItems.map((item, index) => (
              <div key={index} className="px-2">
                <motion.div
                  ...
                  className="bg-gradient-to-tr from-rose-50 to-orange-100 rounded-xl shadow-md overflow-hidden hover:shadow-xl transition hover:scale-105"
                >
                  <img
                    src={item.image}
                    alt={item.title}
      ...
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-xl font-bold mb-6">Budget Friendly Gifts</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {gifts.map((gift) => (
            <div
              key={gift.id}
              className={`${gift.bg} rounded-lg flex flex-col justify-center items-center p-6 h-32 hover:scale-105 transition-transform`}
            >
              <span className={`text-lg font-semibold ${gift.textColor}`}>
                {gift.label}
              </span>
              <span className={`text-2xl md:text-3xl font-bold ${gift.textColor}`}>
                {gift.price}
              </span>
            </div>
          ))}
